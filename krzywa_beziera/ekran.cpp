...
void Ekran::bezier(std::vector< std::pair <int, int> > vect, int N, int r, int g, int b){
    ...
    for (int i=0; i<N; i++){
        t=(double)i/(double)N;
        wzor_x=(pow((1-t), 3)*vect[a-3].first) + (3*pow((1-t), 2)*t*vect[a-2].first) + (3*(1-t)*pow(t, 2)*vect[a-1].first) + pow(t, 3)*vect[a].first;
        wzor_y=(pow((1-t), 3)*vect[a-3].second) + (3*pow((1-t), 2)*t*vect[a-2].second) + (3*(1-t)*pow(t, 2)*vect[a-1].second) + pow(t, 3)*vect[a].second;
        m=t+((double)1.0/(double)N);
        first_point_x = wzor_x;
        first_point_y = wzor_y;
        sec_point_x = (pow((1-m), 3)*vect[a-3].first) + (3*pow((1-m), 2)*m*vect[a-2].first) + (3*(1-m)*pow(m, 2)*vect[a-1].first) + pow(m, 3)*vect[a].first;;
        sec_point_y = (pow((1-m), 3)*vect[a-3].second) + (3*pow((1-m), 2)*m*vect[a-2].second) + (3*(1-m)*pow(m, 2)*vect[a-1].second) + pow(m, 3)*vect[a].second;
        /*qDebug("T: %f /n", t);
...
void Ekran::draw(int N, int r, int g, int b){
    ...
    int j=0;
    for (int i=0; i<N; i++){
        for (auto j=3; j<vect.size(); j+=3) {
            t=(double)i/(double)N;
            wzor_x=(pow((1-t), 3)*vect[j-3].first) + (3*pow((1-t), 2)*t*vect[j-2].first) + (3*(1-t)*pow(t, 2)*vect[j-1].first) + pow(t, 3)*vect[j].first;
            wzor_y=(pow((1-t), 3)*vect[j-3].second) + (3*pow((1-t), 2)*t*vect[j-2].second) + (3*(1-t)*pow(t, 2)*vect[j-1].second) + pow(t, 3)*vect[j].second;
            m=t+((double)1.0/(double)N);
            first_point_x = wzor_x;
            first_point_y = wzor_y;
            sec_point_x = (pow((1-m), 3)*vect[j-3].first) + (3*pow((1-m), 2)*m*vect[j-2].first) + (3*(1-m)*pow(m, 2)*vect[j-1].first) + pow(m, 3)*vect[j].first;;
            sec_point_y = (pow((1-m), 3)*vect[j-3].second) + (3*pow((1-m), 2)*m*vect[j-2].second) + (3*(1-m)*pow(m, 2)*vect[j-1].second) + pow(m, 3)*vect[j].second;
            /*qDebug("T: %f /n", t);
            qDebug("WZOR_X: %f /n", wzor_x);
            qDebug("WZOR_Y: %f /n", wzor_y);
            qDebug("SEC_X: %f /n", sec_point_x);
